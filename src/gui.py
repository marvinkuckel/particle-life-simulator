...
class Button():
    def __init__(self, pos: Tuple[int, int],
        ...
        self.clicked_time = 0  # time when the button was clicked
        self.size_factor = 1  # factor to scale the button size (default-size: 1)
    ...
    def draw(self, screen, mouse_pos):
        ...
        text_surface = self.font.render(self.text, True, (255, 255, 255))  # Render the button text
        text_rect = text_surface.get_rect(center=rect.center)  # Get the text rectangle
        screen.blit(text_surface, text_rect)  # Draw the button text
...
class GUI:
    ...
    def __init__(self, screen, screen_width, screen_height, interaction_matrix, simulation_controlls: dict, padding: int = 60):
        ...
        self.particle_colors = [self.colors[key] for key in ['christmas-green','christmas-red','christmas-gold','christmas-white']]  # Particle colors
        self.interaction_matrix = interaction_matrix
        
        self.buttons = []  # List of buttons
        self.initiate_buttons(simulation_controlls, h_padding = self.padding)  # Initialize buttons

        self.interactions_interface = InteractionsInterface(interaction_matrix, self.particle_colors,  # Initialize interactions interface
                                                            top_left = (screen_width - self.control_panel_width//2,  # Set interactions interface position
                                                            self.buttons[-1].rect.bottom),  # Set interactions interface position
                                                            right = self.screen_width - self.padding)  # Set interactions interface width

        if self.interactions_interface.fields:  # If the interactions interface is not empty ...
            last_field_bottom = list(self.interactions_interface.fields.values())[-1].bottom  # ... get the bottom of the last field ...
            self.instruction_rect = pygame.Rect(self.screen_width - self.control_panel_width + 10,  # ... and set the instruction rectangle ...
            last_field_bottom + 20, self.control_panel_width - 120, 250)  # ... to the bottom of the last field

        # If self.control_panel_width is larger, move self.instruction_rect to the right; if smaller, move it to the left.
        # If last_field_bottom is greater, move self.instruction_rect down; if smaller, move it up.
        # If self.control_panel_width changes, adjust the width of self.instruction_rect accordingly.
    
        else:  # Otherwise ...
            self.instruction_rect = pygame.Rect(self.screen_width - self.control_panel_width + 10,  # ... set the instruction rectangle ...
                                                180, self.control_panel_width - 20, 250)  # ... to the bottom of the last field ...
    ...
    def draw_instruction(self):
        ...
        font = pygame.font.Font(None, 23)  # Define font
        header_font = pygame.font.Font(None, 28)  # Define header font
        header_font.set_bold(True)  # Set bold
        header_font.set_italic(True)  # Set italic
        ...
        y_offset = self.instruction_rect.top + 70  # Move y_offset down by 70 pixels
        ...
        for line in instruction_parts:  # Iterate over each line in the instruction_parts list (each instruction line)
            ...
            # After finishing the line, update the y_offset to move down for the next line of text, with an additional vertical spacing of ... pixels
    ...
    def initiate_buttons(self, simulation_controlls, h_padding = 60):
        """
        Initializes the buttons for the control panel.
        """
    ...
    def button_click(self, event):
        """
        Checks if the mouse is over a button and triggers the corresponding action.
        """
        self.interactions_interface.handle_click(event)  # Handle click events on the interactions interface
        
        for button in self.buttons:  # Iterate over the buttons
            if button.rect.collidepoint(event.pos):  # Check if the mouse is over the button
                button.trigger(event)  # Trigger the button with the specified event
    ...
    def draw_control_panel(self, mouse_pos):
        ...
        for button in self.buttons:  # Iterate over the buttons
            button.draw(self.screen, mouse_pos)  # Draw the button with the current mouse position
            
        self.interactions_interface.draw(self.screen)  # Draw the interactions interface
